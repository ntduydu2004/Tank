...
export class MainMenuScreen extends Screen {
    ...
    private createBackground() {
        this.background = this.scene.add.image(this.scene.sys.canvas.width / 2, this.scene.sys.canvas.height / 2, 'background').setInteractive()
        this.title = this.scene.add.bitmapText(this.scene.sys.canvas.width / 2, this.scene.sys.canvas.height *0.3, 'font', 'KING OF TANK', 50).setOrigin(0.5).setTint(0x59a800)
        this.particles = this.scene.add.particles(this.scene.sys.canvas.width / 2, this.scene.sys.canvas.height *0.3, 'flares', {
            frame: ['white'],
