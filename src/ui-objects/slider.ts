...
export class Slider extends Phaser.GameObjects.Container {
    ...
    public constructor(params: ISliderConstructor) {
        ...
        this.text = this.scene.add
            .bitmapText(-100, 0, 'font', params.text, 30)
            .setTint(0xffffff)
            .setOrigin(0.5)

        this.value = this.scene.add
            .text(500, 0, Math.floor(params.value).toString(), {
                color: 'white',
                fontSize: '40px',
                fontStyle: 'bold',
            })
            .setOrigin(0.5)
        this.pointer = this.scene.add.circle(
            params.width * (params.value / params.max),
            0,
            10,
            0x00ffff
        )
        ...
        this.pointer.on('drag', (pointer: Phaser.Input.Pointer, dragX: number, dragY: number) => {
            dragX = Phaser.Math.Clamp(
                dragX,
                this.sliderGraphics.x,
                this.sliderGraphics.x + params.width
            )
